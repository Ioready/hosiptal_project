...
<body>
    ...
    $(document).ready(function() {
        ...
        function updateTableHeaders(selectedNames) {
            ...
            var headerRow = $('#practitioner_id tr');
            headerRow.empty();
            headerRow.append('<th class="text-center fw-bold" style="font-size:16px; background-color:#337ab7;color:white;">Time</th>');
            ...
            selectedNames.forEach(function(practitionerId) {
                ...
                headerRow.append('<th class="day-cell text-center" style="font-size:14px;background-color:#337ab7;color:white;">' + practitionerName + '</th>');
            });
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateTableHeaders(selectedNames) {
                ...
                selectedNames.forEach(function(practitionerId) {
                    var practitionerName = $('#departmentanddoctordata option[value="' + practitionerId + '"]').text();
                    headerRow.find('tr').append('<th class="day-cell text-center" style="font-size:14px;background-color:#337ab7;color:white;" data-practitioner-id="' + practitionerId + '">' + practitionerName + '</th>');
                });
            ...
            function filterAppointments() {
                ...
                if (selectedDate) {
                    fetchAppointments(selectedDate, selectedPractitioners);
                } else {
                    $('.appointment-row').hide();
                }
            }

            function fetchAppointments(date, practitioners) {
                console.log('Fetching appointments for date:', date, 'and practitioners:', practitioners); // Debug
                $.ajax({
                    url: 'your-api-endpoint', // Replace with your API endpoint
                    type: 'GET',
                    data: { date: date, practitioners: practitioners },
                    success: function(response) {
            ...
            function populateAppointments(data) {
                $(".appointment-row").remove(); // Clear previous appointments

                data.forEach(function(appointment) {
                    var startTime = appointment.start_time;
                    var endTime = appointment.end_time;
                    var practitionerId = appointment.practitioner_id;
                    var comment = appointment.comment;

                    var startTimeFormatted = formatTime(startTime);
                    var endTimeFormatted = formatTime(endTime);

                    var rows = calculateRowSpan(startTime, endTime);

                    var rowSelector = '.day-cell[data-time="' + startTimeFormatted + '"][data-day="' + practitionerId + '"]';

                    var appointmentHtml = '<td class="day-cell appointment-row" rowspan="' + rows + '" data-date="' + appointment.date + '" data-day="' + practitionerId + '">' +
                                            '<label style="background-color:green; text-align: center; border: 2px solid; border-radius: 5px; padding: 11px;">' +
                                            '<span style="background-color: green; color: white;"><strong>' + appointment.patient_name + '</strong><br>' + comment + '<br>' + startTimeFormatted + ' - ' + endTimeFormatted + '</span>' +
                                            '</label>' +
                                          '</td>';
                    
                    $(rowSelector).after(appointmentHtml);
                });
            }
            ...
            function calculateRowSpan(startTime, endTime) {
                ...
                return (end - start) / (60 * 1000); // Calculate row span in minutes
            }
            ...
            window.getLocations = function(view_id) {
                $.ajax({
                    ...
                    data: { id: view_id },
                    success: function(response) {
                        console.log('Location filter response:', response); // Debug
                        $('#departmentanddoctordata').empty().append('<option value="">All Select</option>');
                        $.each(response, function(key, value) {
                            var name = value.name || '';
                            var firstName = value.first_name || '';
                    ...
                    error: function(xhr, status, error) {
                        console.error('Location filter error:', error); // Debug
                    }
                });
            };
